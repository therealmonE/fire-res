...
@Slf4j
public class SampleSecondGroupThermocoupleBoundEnricher implements SampleEnricher<UnheatedSurfaceReport, UnheatedSurfaceSample> {
    ...
    public void enrich(UnheatedSurfaceReport report, UnheatedSurfaceSample sample) {
        log.info("Unheated surface: enriching sample {} second group with thermocouple bound", sample.getId());
        ...
        sample.getSecondGroup().setThermocoupleBound(thermocoupleBound);
    }

    @Override
    public boolean supports(EnrichType enrichType) {
        return SAMPLE_SECOND_GROUP_THERMOCOUPLE_BOUND.equals(enrichType);
    }

    @Override
    public List<EnrichType> getAffectedTypes() {
        return List.of(SAMPLE_SECOND_GROUP_MEAN_WITH_THERMOCOUPLE_TEMPERATURES);
    }
